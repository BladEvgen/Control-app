...
import { persistFileConsent, readFileConsent } from "../faceLab/faceLabConsentStorage";
import {
  humanizeApiError,
...
const DEFAULT_OUTCOME_HEADLINE: Record<Exclude<SessionOutcome, "idle">, string> = {
  success: "Готово",
...
function evaluateFaceLabSession(
  ...
  if (padWarning) {
    ...
    return {
      ...
      headline:
        mode === "search"
          ? "Живость кадра не подтверждена"
          : "Проверка кадра не прошла",
    };
  }
  ...
  if (pad === null) {
    return {
      outcome: "fail",
      message: "Живость на сервере не проверена — переснимите и отправьте снова.",
      headline: mode === "search" ? "PAD не отработал" : undefined,
    };
  }
  ...
  if (mode === "compare") {
    const v = parseVerifyResponse(verifyPayload);
    if (!v) {
      const errRaw =
      ...
      return { outcome: "fail", message: [f.title, f.detail].filter(Boolean).join(" ") };
    }

    if (v.verified && padStrong) {
      return {
        outcome: "success",
        message:
          v.relaxed_match === true
            ? "Эталон совпал, живость ок (сходство на грани порога)."
            : "Эталон совпал, живость подтверждена.",
      };
    }

    if (v.verified && padWeak) {
      return {
        outcome: "partial",
        message: "Эталон совпал, живость без уверенного «да» — лучше переснять.",
      };
    }
    ...
    if (!v.verified && padStrong) {
      return {
        outcome: "fail",
        message: "Живость ок, но с эталоном не совпало — другой человек или ракурс.",
      };
    }

    return {
      outcome: "fail",
      message: "И эталон, и живость без подтверждения — переснимите.",
    };
  }
...
const FaceLabPage: React.FC = () => {
  ...
  const [voiceLang, setVoiceLang] = useState<FaceLabVoiceLang>(() => readVoiceLang());
    ...
    try {
      try {
        ...
        if (axios.isAxiosError(padErr) && padErr.response?.data) {
          ...
          padWarnLocal =
            padErr instanceof Error ? padErr.message : "Сбой проверки живости";
        }
  ...
  const staffListErrorFriendly = staffListError ? humanizeApiError(staffListError) : null;
  const padWarningFriendly = padWarning ? humanizePadFailureReason(padWarning) : null;
    ...
    <div className="mx-auto w-full max-w-[88rem] px-3 py-6 text-slate-900 dark:text-slate-100 sm:px-5 sm:py-8 md:px-8 lg:px-10 lg:py-10 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
      ...
      <header className="mb-6 border-b border-slate-200 pb-5 dark:border-slate-800/90 sm:mb-8 sm:pb-6">
        ...
        <p className="mt-2 max-w-xl text-sm text-slate-600 dark:text-slate-400">
          Живость, поиск по базе или сравнение с эталоном.
        </p>
      </header>
      ...
      >
        <p>Файл уйдёт на сервер только после «Отправить на проверку».</p>
      </FaceLabConsentDialog>
      ...
      <section className="rounded-2xl border border-slate-200/90 bg-white/95 p-4 shadow-md shadow-slate-200/30 dark:border-slate-600/80 dark:bg-slate-900/55 dark:shadow-black/25 sm:p-5 md:p-6">
        ...
        <Box className="relative rounded-xl border border-slate-200/80 bg-slate-100/90 p-1 dark:border-slate-600 dark:bg-slate-950">
          <motion.div
            ...
            initial={false}
            animate={{
              left: mode === "search" ? 4 : "calc(50% + 2px)",
              width: "calc(50% - 6px)",
            }}
            transition={{ type: "spring", stiffness: 420, damping: 34 }}
          />
          <div className="relative z-10 grid min-h-[3rem] grid-cols-2 gap-0">
            <button
              type="button"
              className={`flex min-h-[3rem] items-center justify-center gap-2 rounded-lg px-2 py-2.5 text-center text-sm font-semibold leading-snug transition-colors sm:min-h-0 sm:py-3.5 sm:text-base ${
                mode === "search"
            ...
            >
              <FaSearch className="h-4 w-4 shrink-0 opacity-80" aria-hidden />
              Поиск в галерее
            </button>
            <button
              type="button"
            ...
            >
              <FaUserCheck className="h-4 w-4 shrink-0 opacity-80" aria-hidden />
              С эталоном
            </button>
          </div>
        </Box>
        ...
        <section className="rounded-2xl border border-slate-200/90 bg-white/95 p-4 shadow-md shadow-slate-200/30 dark:border-slate-600/80 dark:bg-slate-900/55 dark:shadow-black/25 sm:p-5 md:p-6">
          ...
          {staffListError ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-800/40 dark:bg-amber-950/20 dark:text-amber-100">
              <p className="font-medium">{staffListErrorFriendly?.title ?? staffListError}</p>
              {staffListErrorFriendly?.detail ? (
                <p className="mt-1 text-amber-900/90 dark:text-amber-200/90">{staffListErrorFriendly.detail}</p>
              ) : null}
            </div>
          ) : allStaffLoading ? (
            <LoaderComponent
            ...
            <p className="text-sm text-slate-600 dark:text-slate-500">
              Нет ни одного сотрудника с привязкой к отделу. Проверьте права или настройки.
            </p>
          ) : (
      ...
      >
        ...
        >
          ...
          <ToggleButtonGroup
            ...
            aria-label="Озвучка подсказок камеры"
            sx={{
              flexWrap: "wrap",
              gap: 0.5,
        ...
        {previewUrl ? (
          <Box className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-950/80 md:p-5">
            ...
            >
              ...
              <Typography variant="caption" fontWeight={600} letterSpacing={0.06} textTransform="uppercase">
                Превью
              </Typography>
            </Stack>
            <Box className="flex flex-col items-center gap-4 md:mx-auto md:max-w-3xl">
              ...
              >
                <FaImage className="h-4 w-4 shrink-0 text-primary-600 dark:text-primary-400" aria-hidden />
                <div className="min-w-0 text-center">
                  <Typography variant="body2" fontWeight={600} className="truncate text-slate-900 dark:text-slate-100">
                    {file?.name ?? "файл"}
                  </Typography>
                  {file ? (
      ...
      {sessionOutcome !== "idle" ? (
        <motion.section
          ...
          transition={{ type: "spring" as const, stiffness: 300, damping: 22, mass: 0.9 }}
        >
          <motion.p
      ...
      {padWarning && padWarningFriendly ? (
        ...
        >
          <p className="font-medium text-amber-900 dark:text-amber-200">{padWarningFriendly.title}</p>
          {padWarningFriendly.detail ? (
            <p className="mt-1 text-amber-800/90 dark:text-amber-200/85">{padWarningFriendly.detail}</p>
          ) : null}
        </motion.div>
      ) : null}
      ...
      {error && errorFriendly ? (
        <motion.div
          ...
          transition={{ type: "spring", stiffness: 340, damping: 26 }}
        >
          <p className="font-medium text-amber-950 dark:text-amber-100">{errorFriendly.title}</p>
          {errorFriendly.detail ? (
            <p className="mt-2 text-sm text-amber-800/90 dark:text-amber-200/90">{errorFriendly.detail}</p>
          ) : null}
        </motion.div>
      ) : null}
      ...
      {verifyResult !== null ? (
        <motion.section
          ...
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ type: "spring", stiffness: 300, damping: 23, delay: 0.04 }}
        >
          <motion.h2
            className="text-xs font-semibold uppercase tracking-wider text-slate-500"
            initial={{ opacity: 0 }}
