...
async def create_lesson_attendance(session, staff_pin):
    data = {"attendance_data": [generate_random_data(staff_pin)]}
    async with session.post(create_lesson_url, headers=headers, json=data) as response:
        if response.status == 202:
            ...
            log_message(f"Ошибка создания записи для {staff_pin}: {response.status} - {await response.text()}")
            return None
    ...
    while retries < max_retries:
        async with session.get(f"{check_task_url}{task_id}/", headers=headers) as response:
            if response.status == 200:
                ...
                log_message(f"Ошибка проверки статуса задачи {task_id}: {response.status} - {await response.text()}")
...
async def update_lesson_attendance(session, lesson_id):
    ...
    async with session.put(f"{update_lesson_url}{lesson_id}/", headers=headers, json=data) as response:
        if response.status == 200:
            log_message(f"Запись с ID {lesson_id} успешно обновлена.")
        else:
            log_message(f"Ошибка обновления записи {lesson_id}: {response.status} - {await response.text()}")
...
async def run_tests():
    staff_members = [
        "s7677", "s756790", "s22311", "s100323", "s00375",
        "s00260", "s00226", "s00173", "s00105", "s00018", "S15213213231132",
    ]
