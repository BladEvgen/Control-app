...
class PhotoConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def get_photos_for_date(self, date_str):
        try:
            ...
            attendances_qs = models.LessonAttendance.objects.filter(date_at=date).select_related('staff__department').values(
                'id',
                'first_in',
                'staff_image_path',
                'tutor',
            ...
            photos = []
            async for att in attendances_qs:
    ...
    async def get_attendance_data_by_id(self, attendance_id):
        try:
            attendance_qs = models.LessonAttendance.objects.filter(id=attendance_id).select_related('staff__department').values(
                'id',
                'first_in',
                'staff_image_path',
                'tutor',
            ...
            attendance_data = await attendance_qs.afirst()
