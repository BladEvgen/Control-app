...
def get_absence_status(staff, date):
    """Проверка, есть ли отсутствие у сотрудника на указанную дату.
    ...
    Returns:
        models.StaffAbsence or None: Объект отсутствия или None.
    """
    return models.StaffAbsence.objects.filter(
        staff=staff, start_date__lte=date, end_date__gte=date, approved=True
    ).first()


def is_remote_working(staff, date):
    """Проверка, работает ли сотрудник дистанционно на указанную дату.
    ...
    Returns:
        bool: True, если сотрудник работает дистанционно, иначе False.
    """
    permanent_remote = models.RemoteWorkPeriod.objects.filter(
        staff=staff, is_permanent=True, approved=True
    ).exists()

    if permanent_remote:
        return True

    remote_work_period = models.RemoteWorkPeriod.objects.filter(
        staff=staff, start_date__lte=date, end_date__gte=date, approved=True
    ).exists()

    return remote_work_period
