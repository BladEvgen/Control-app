...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        qs = LessonAttendance.objects.filter(
            staff_image_path__isnull=False
        ...
        self.stdout.write(
            self.style.WARNING(
                ...
                f"| force_manual={force_manual} | device={device} "
        ...
        for start in range(0, total, batch_size):
            ...
            for record in batch:
                ...
                if not dry_run:
                    LessonAttendance.objects.filter(pk=record.pk).update(
